...
- （1）文件头header:描述ELF文件的基本信息，比如版本号，目标机器型号，程序入口地址。
  ...
  ELF Header:
    ...
    Start of section headers:          5528 (bytes into file)  // 段表位置
    Flags:                             0x0
    Size of this header:               64 (bytes)
    Size of program headers:           0 (bytes)
    Number of program headers:         0
    Size of section headers:           64 (bytes)         // 段表大小
    Number of section headers:         25                 // 段表数目
...
- （2）段表（Section Header Table）：描述各个段（.text .data .bss段）的信息，比如每个段的名字，段的长度，段在文件中的偏移，读写权限以及其他权限
```c
 1，ojdump -h   // 查看主要段的信息
...
name是段名；type为段类型；Address是该段被加载后在地址空间的虚拟地址，否则为0（可执行文件不能为0） align段地址对齐（2的指数幂）
```
